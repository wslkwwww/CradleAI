...
export interface ScenePromptParams {
  ...
  conversationHistory?: string; // Add conversation history parameter
  characterJsonData?: string;   // Add character jsonData parameter
}
...
严格按以下格式用中文回复，不要包含任何其他文字：
{
   ...
   */
  selfPost: (params: ScenePromptParams) => `这是你自己发布的朋友圈动态，现在有人回复了你的帖子：
...
回复，不要包含任何其他文字：
{
   ...
   */
  continuedConversation: (params: ScenePromptParams) => `你正在朋友圈中与${params.authorName || '某人'}进行对话。
...
作为角色 ${params.charDescription.substring(0, 50)}，请根据你的性格特点和上方的历史对话上下文，回应最新的回复。请保持对话的连贯性，并确保你的回应风格与你的角色设定一致。
