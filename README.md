...
2.支持配置OPENAI兼容渠道（不支持贩子的不规范格式）/hajimi轮询/claw轮询渠道，可以设置自己的中转或者轮询

3.修复了一些局部功能尚不支持openrouter渠道的问题

4.修复了<summary><detail>标签不能渲染的问题（原先使用包含此类状态栏标签的预设，会“吞消息”）

5.修复了openai兼容渠道消息截断的问题（目前暂时没有设置maxtoken）

6.修复了api设置页面切换后无法正确进行测试和应用设置的问题
0502：
