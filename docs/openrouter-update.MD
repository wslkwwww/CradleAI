...
*   当前 App的聊天消息 直接与 OpenRouter API 交互，存在安全风险（API Key 暴露）和难以集中管理的问题。 本项目旨在通过引入服务器端代理和用户鉴权机制，提高 App 的安全性和可维护性，并为后续功能扩展奠定基础。
...
并安全地在数据库中存储这些key，它们将在后续的流程中，被调取并且被轮询
