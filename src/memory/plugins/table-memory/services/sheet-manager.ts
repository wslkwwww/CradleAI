  ...
  "tableActions": [
    {
      ...
      "rowData": {"0": "第一列的值", "1": "第二列的值"}
        ...
        // 确保对话内容包含在提示词中
        // 修复: 改进对对话内容的检查逻辑，确保对话内容总是被包含
        let dialogIncluded = false;
        
        // 检查提示词中是否已包含对话内容的位置标记 
        if (userPrompt.includes('对话内容:')) {
          // 如果提示词模板中已经有对话内容标记，检查对话内容是否实际插入了
          const contentAfterMarker = userPrompt.split('对话内容:')[1];
          if (contentAfterMarker && 
              (contentAfterMarker.includes(chatContent?.substring(0, 30)) || 
               chatContent === '' || 
               contentAfterMarker.includes('对话内容为空'))) {
            dialogIncluded = true;
          } else {
            // 如果标记后没有正确插入对话内容，重新构建这部分
            userPrompt = userPrompt.replace(/对话内容:[\s\S]*?(?=\n\n请提供|$)/m, 
              `对话内容:\n${chatContent || '对话内容为空，请基于当前表格内容进行更新或优化。'}`);
            dialogIncluded = true;
          }
        }
        ...
        userPrompt = userPrompt
          .replace(/<user>/g, options.userName || '用户')
          .replace(/<ai>/g, options.aiName || 'AI');
        // 添加日志记录完整的请求体内容
        console.log(`[TableMemory] 发送给LLM的systemPrompt内容(前100字符):\n${systemPrompt.substring(0, 100)}...`);
        console.log(`[TableMemory] 发送给LLM的userPrompt内容(前100字符):\n${userPrompt.substring(0, 100)}...`);
        // 检查提示词中是否包含对话内容
        if (chatContent && chatContent.length > 0) {
          if (userPrompt.includes(chatContent.substring(0, 30))) {
            console.log(`[TableMemory] 确认: 对话内容已正确包含在提示词中`);
          } else {
            console.warn(`[TableMemory] 警告: 对话内容可能未正确包含在提示词中!`);
            // 输出完整用户提示，便于调试
            console.log(`[TableMemory] 用户提示词完整内容(调试用):\n${userPrompt}`);
          }
        }
