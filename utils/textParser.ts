...
export function containsComplexHtml(text: string): boolean {
  ...
  // Also detect media elements (audio, video)
  if (text.includes('<audio') || text.includes('</audio>') || 
      text.includes('<video') || text.includes('</video>')) {
    return true;
  }
  
  // Detection for SVG elements
  if (text.includes('<svg') || text.includes('</svg>')) {
    return true;
  }
  
  // Detect web fonts or stylesheets
  if (text.includes('@font-face') || 
      text.includes('@import url') || 
      text.includes('url(') && text.includes('fonts.googleapis.com')) {
    return true;
  }
...
export function preserveFormatting(text: string): string {
  ...
  // Convert newlines to <br> tags and preserve consecutive spaces
  return text
    .replace(/\n/g, '<br>')
    .replace(/ {2,}/g, (match) => {
...
export function formatCodeBlocks(text: string): string {
  ...
  return text.replace(/```(html|css|javascript|js|jsx|tsx|ts)?([\s\S]*?)```/g, (match, language, code) => {
    // Clean up the code - remove first empty line if present
    const cleanCode = code.trim().replace(/^\n/, '');
    
    // If it's an HTML block, we may want to render it directly
    if (language === 'html' && cleanCode.includes('<')) {
      // Check if it seems to be complete HTML (has html and body tags)
      if ((cleanCode.includes('<html') || cleanCode.includes('<!DOCTYPE')) && cleanCode.includes('<body')) {
        // Extract just the body content to avoid nesting HTML documents
        const bodyMatch = cleanCode.match(/<body[^>]*>([\s\S]*)<\/body>/);
        if (bodyMatch && bodyMatch[1]) {
          return bodyMatch[1].trim();
        }
      }
      
      // Otherwise just return the HTML directly
      return cleanCode;
    }
    
    // For other languages, wrap in pre/code blocks
